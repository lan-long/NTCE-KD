...
def main(cfg, resume, opts):
    ...
    if opts:
        addtional_tags = ["{}_{}".format(k, v) for k, v in zip(opts[::2], opts[1::2]) if "EXPERIMENT.TAG" not in k]
        if len(addtional_tags):
            tags += addtional_tags
...
if __name__ == "__main__":
    ...
    if cfg.NTCE.ISAUG:
        cfg.SOLVER.TRAINER += "_aug"
        cfg.SOLVER.EPOCHS = int(cfg.SOLVER.EPOCHS * 2)
        cfg.SOLVER.WEIGHT_DECAY = cfg.SOLVER.WEIGHT_DECAY * 0.5
        cfg.SOLVER.LR_DECAY_STAGES = [int(i * 2) for i in cfg.SOLVER.LR_DECAY_STAGES]
    cfg.freeze()
